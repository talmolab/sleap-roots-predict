...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        try:
            ...
            # subclasses OSError) now get this line too. Accepted — the exit code is 1
            # either way and the traceback still surfaces. Log a clean message, then
            # re-raise so the process still exits via Python's default
            # unhandled-exception code (1).
