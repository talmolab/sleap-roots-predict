...
- Add dependencies: **`sleap-roots-contracts==0.1.0a3`** (`ModelCard`, `ModelRef`,
  `ResolvedParams`, `RootType`) and **`wandb`**.
- Add a **`@pytest.mark.wandb`** marker, deselected by default (like `gpu`/`acceptance`).
...
- **Dependency/sequencing:** `sleap-roots-contracts==0.1.0a3` is merged and being released; the
  TDD implementation begins once it is published and pinnable. Proposal + spec work needs no install.
