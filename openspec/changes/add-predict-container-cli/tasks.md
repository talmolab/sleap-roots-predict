...
- [x] 1.1 Promote the `LocalCardSource` model fixtures to `tests/conftest.py`: `rice_source`
...
- [x] 1.2 Create the committed fixture input scan dir `tests/assets/scans/<scan_key>/` (pick a
...
- [x] 2.1 **Check first:** a `tomllib` guard test asserts `[project.scripts]` declares
  `sleap-roots-predict = "sleap_roots_predict.__main__:main"` (RED).
- [x] 2.2 Add the `[project.scripts]` entry; confirm `uv sync --frozen` still passes (console
  scripts are not resolution inputs — no lock change expected); re-lock only if uv requires it.
- [x] 2.3 **Check first:** a CI-safe test asserts `python -m sleap_roots_predict --help` parses
  two positional args (`input_dir`, `output_dir`) and exits 0 without touching the network (RED).
- [x] 2.4 Create `sleap_roots_predict/__main__.py`: `argparse` (`input_dir`, `output_dir`),
...
- [x] 3.1 **Check first:** `discover_scans(fixture_dir)` returns the scan with `scan_key`, its
  frame paths (only image files; `.json`/non-image files excluded), and a
  `ResolvedParams(species=rice, mode=cylinder, age=3)` (RED).
- [x] 3.2 Implement `discover_scans`: `rglob "*.scan_metadata.json"`; `scan_key` = stem; frames
...
- [x] 3.3 **Check first + implement (separate RED tests):** (a) non-image file present is
  ignored as a frame; (b) `test_batch_does_not_import_trait_extractor` guard (mirrors #16's
  no-`sleap-roots`-import test).
- [x] 3.4 **Check first + implement (separate RED tests):** stem ≠ sidecar `scan_key` → that
...
- [x] 4.1 **Check first (real inference):** `run_batch(fixture_in, tmp_out,
...
- [x] 4.2 Implement `run_batch(input_dir, output_dir, *, source=None, peak_threshold=0.2,
...
- [x] 4.3 **Check first:** a two-scan batch (separate dirs) writes **both** scans'
  ...
  (RED → green).
- [x] 4.4 **(atomic commit)** Export `run_batch` from `sleap_roots_predict/__init__.py` and
...
- [x] 5.1 **Check first:** assert the video `run_batch`/`discover_scans` builds is 1-channel
...
- [x] 6.1 **Check first:** a second `run_batch` over the same `output_dir` skips a scan whose
...
- [x] 6.2 Implement the skip check before per-scan inference.

## 7. Failure isolation + exit codes

- [x] 7.1 **Check first:** a batch with one broken scan (unreadable/absent frames) →
...
- [x] 7.2 Implement per-scan `try/except` isolation + `BatchResult` + zero-model→failed +
  empty-input warn.
- [x] 7.3 **Check first:** `main([in, out])` returns `0` when all scans pass and `1` when any
...
- [x] 8.1 **Check first (manual gate, documented + reproducible):**
...
- [x] 8.2 Replace the REPL stub: exec-form ENTRYPOINT and **remove the stale `CMD`**;
...
- [x] 9.1 **Check first:** a guard test (or documented check) asserts `docker-build.yml` passes
  `build-args: SRP_PREDICT_CODE_SHA=${{ github.sha }}` and tags `type=sha,format=long`.
- [x] 9.2 Add the `build-args` block; set `type=sha,format=long` (so the `sha-<full>` tag equals
...
- [x] 10.1 Update docs (respecting the repo's SSOT direction — `openspec/project.md` for module
...
- [x] 10.2 Run `black`, `ruff check`, `codespell` (scans the whole tree — sidecar JSON + docs
  must be typo-clean); `pytest` (CPU suite green); the `gpu` subset + the docker-run gate via
  `/pre-merge`.
- [x] 10.3 `openspec validate add-predict-container-cli --strict` passes.
